(function(){"use strict";class s{constructor(t){this.config=t,this.vapi=null,this.isCallActive=!1,this.eventListeners=new Map,this.loadVapiSDK()}async loadVapiSDK(){try{if(typeof window.Vapi>"u"&&await this.loadScript("https://cdn.jsdelivr.net/npm/@vapi-ai/web@latest/dist/index.js"),typeof window.Vapi>"u")throw new Error("Failed to load Vapi SDK");this.vapi=new window.Vapi(this.config.apiKey),this.setupEventListeners()}catch(t){throw console.error("Failed to load Vapi SDK:",t),new Error("Voice service unavailable. Please try again later.")}}loadScript(t){return new Promise((e,i)=>{if(document.querySelector(`script[src="${t}"]`)){e();return}const r=document.createElement("script");r.src=t,r.onload=e,r.onerror=()=>i(new Error(`Failed to load script: ${t}`)),r.ontimeout=()=>i(new Error(`Script load timeout: ${t}`)),setTimeout(()=>{i(new Error(`Script load timeout: ${t}`))},1e4),document.head.appendChild(r)})}setupEventListeners(){this.vapi&&(this.vapi.on("call-start",()=>{this.isCallActive=!0,this.emit("call-start")}),this.vapi.on("speech-start",()=>{this.emit("speech-start")}),this.vapi.on("speech-end",()=>{this.emit("speech-end")}),this.vapi.on("call-end",()=>{this.isCallActive=!1,this.emit("call-end")}),this.vapi.on("error",t=>{console.error("Vapi error:",t),this.isCallActive=!1,this.emit("error",t)}),this.vapi.on("volume-level",t=>{this.emit("volume-level",t)}),this.vapi.on("message",t=>{this.emit("message",t)}))}async start(){if(!this.vapi)throw new Error("Vapi client not initialized");if(this.isCallActive){console.warn("Call is already active");return}try{await this.vapi.start({model:{provider:"openai",model:"gpt-3.5-turbo",messages:[{role:"system",content:"You are a helpful assistant. Keep responses brief and natural for a phone conversation."}]},voice:{provider:"azure",voiceId:"andrew"},...this.config.phoneNumber&&{phoneNumber:this.config.phoneNumber}})}catch(t){throw console.error("Failed to start Vapi call:",t),t}}stop(){if(!(!this.vapi||!this.isCallActive))try{this.vapi.stop()}catch(t){console.error("Failed to stop Vapi call:",t)}}on(t,e){this.eventListeners.has(t)||this.eventListeners.set(t,[]),this.eventListeners.get(t).push(e)}off(t,e){if(!this.eventListeners.has(t))return;const i=this.eventListeners.get(t),o=i.indexOf(e);o>-1&&i.splice(o,1)}emit(t,e){this.eventListeners.has(t)&&this.eventListeners.get(t).forEach(i=>{try{i(e)}catch(o){console.error(`Error in event listener for ${t}:`,o)}})}destroy(){this.vapi&&(this.stop(),this.vapi=null),this.eventListeners.clear()}}class a{constructor(t={}){this.config={apiKey:"",vapiKey:"",phoneNumber:"",position:"bottom-right",backgroundColor:"#FFFF00",iconColor:"#000000",mockMode:!1,...t},this.vapi=null,this.widgetElement=null,this.buttonElement=null,this.isInitialized=!1,this.currentCallState="idle",this.retryCount=0,this.maxRetries=3,this.init()}init(){if(this.isInitialized){console.warn("Buzzwald widget is already initialized");return}try{this.validateConfig(),this.checkBrowserSupport(),this.injectStyles(),this.createWidget(),this.initializeVapi(),this.isInitialized=!0}catch(t){console.error("Buzzwald: Failed to initialize widget",t),this.handleInitializationError(t)}}validateConfig(){if(!this.config.vapiKey)throw new Error("Vapi key is required");if(typeof this.config.vapiKey!="string")throw new Error("Vapi key must be a string");["bottom-right","bottom-left","top-right","top-left"].includes(this.config.position)||(console.warn(`Buzzwald: Invalid position "${this.config.position}". Using default "bottom-right"`),this.config.position="bottom-right"),this.config.backgroundColor&&!this.isValidColor(this.config.backgroundColor)&&(console.warn(`Buzzwald: Invalid background color "${this.config.backgroundColor}". Using default`),this.config.backgroundColor="#FFFF00"),this.config.iconColor&&!this.isValidColor(this.config.iconColor)&&(console.warn(`Buzzwald: Invalid icon color "${this.config.iconColor}". Using default`),this.config.iconColor="#000000")}isValidColor(t){const e=document.createElement("div");return e.style.color=t,e.style.color!==""}checkBrowserSupport(){if(!window.fetch)throw new Error("Browser does not support fetch API");if(!window.Promise)throw new Error("Browser does not support Promises");if(!window.addEventListener)throw new Error("Browser does not support addEventListener")}handleInitializationError(t){const e=document.createElement("div");e.className="buzzwald-widget-error",e.style.cssText=`
      position: fixed;
      bottom: 20px;
      right: 20px;
      width: 60px;
...
Please check the console for more details.`)}),document.body.appendChild(e),this.errorWidget=e}injectStyles(){const t="buzzwald-widget-styles";if(document.getElementById(t))return;const e=document.createElement("style");e.id=t,e.textContent=`
    ...
    `,e.textContent=t,document.body.appendChild(e),setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},3e3)}initializeVapi(){this.config.mockMode?this.vapi=this.createMockVapiClient():this.vapi=new s({apiKey:this.config.vapiKey,phoneNumber:this.config.phoneNumber}),this.vapi.on("call-start",()=>this.updateCallState("connecting")),this.vapi.on("speech-start",()=>this.updateCallState("connected")),this.vapi.on("call-end",()=>{this.updateCallState("ended"),setTimeout(()=>this.updateCallState("idle"),2e3)}),this.vapi.on("error",t=>{console.error("Buzzwald: Vapi error",t),this.updateCallState("ended"),setTimeout(()=>this.updateCallState("idle"),2e3)})}createMockVapiClient(){return{eventListeners:new Map,async start(){console.log("🎭 Mock: Starting Vapi call"),this.emit("call-start"),setTimeout(()=>this.emit("speech-start"),2e3),setTimeout(()=>{console.log("🎭 Mock: Auto-ending call"),this.emit("call-end")},1e4)},stop(){console.log("🎭 Mock: Stopping call"),this.emit("call-end")},on(t,e){this.eventListeners.has(t)||this.eventListeners.set(t,[]),this.eventListeners.get(t).push(e)},off(t,e){if(!this.eventListeners.has(t))return;const i=this.eventListeners.get(t),o=i.indexOf(e);o>-1&&i.splice(o,1)},emit(t,e){this.eventListeners.has(t)&&this.eventListeners.get(t).forEach(i=>{try{i(e)}catch(o){console.error(`Error in event listener for ${t}:`,o)}})},destroy(){this.eventListeners.clear()}}}updateCallState(t){switch(this.currentCallState=t,this.buttonElement.classList.remove("connecting","connected","ended"),t!=="idle"&&this.buttonElement.classList.add(t),t){case"connecting":this.buttonElement.setAttribute("aria-label","Connecting...");break;case"connected":this.buttonElement.setAttribute("aria-label","End call");break;case"ended":this.buttonElement.setAttribute("aria-label","Call ended");break;default:this.buttonElement.setAttribute("aria-label","Start phone call")}}destroy(){try{this.vapi&&(this.vapi.destroy(),this.vapi=null),this.widgetElement&&(this.widgetElement.remove(),this.widgetElement=null),this.errorWidget&&(this.errorWidget.remove(),this.errorWidget=null);const t=document.getElementById("buzzwald-widget-styles");t&&t.remove(),this.retryTimeout&&(clearTimeout(this.retryTimeout),this.retryTimeout=null),this.isInitialized=!1}catch(t){console.error("Buzzwald: Error during widget destruction",t)}}}(function(){if(window.buzzwaldWidget){console.warn("Buzzwald widget is already initialized");return}function n(){try{const t=window.BuzzwaldConfig||{};if(!t.mockMode&&!t.vapiKey){console.error("Buzzwald: vapiKey is required in window.BuzzwaldConfig");return}window.buzzwaldWidget=new a(t),window.buzzwaldDestroy=function(){window.buzzwaldWidget&&(window.buzzwaldWidget.destroy(),window.buzzwaldWidget=null,window.buzzwaldDestroy=null)},console.log("Buzzwald widget initialized successfully")}catch(t){console.error("Failed to initialize Buzzwald widget:",t)}}if(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",n):n(),!window.BuzzwaldConfig){let t=setInterval(()=>{window.BuzzwaldConfig&&!window.buzzwaldWidget&&(clearInterval(t),n())},100);setTimeout(()=>{clearInterval(t)},1e4)}})()})();
